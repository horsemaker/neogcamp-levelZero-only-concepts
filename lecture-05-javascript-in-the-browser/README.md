...
- If the Javascript code  is to be executed first or along with the HTML code then the js code is preferred to be written within the script tag in the head tag
- Else, if two or more HTML files require the same set of JS code, then it is preferred to create a separate JS file and link the HTML pages via the ‘src’ attribute

## DOM methods
### Event Listeners
- Event - Events represent the change in the state of an element such that some activity is performed by the user or by the browser on the element. A case-sensitive string representing the [event type](https://developer.mozilla.org/en-US/docs/Web/Events) to listen for.
- The [EventListener]() interface represents an object that can handle an event dispatched by an EventTarget object
- For this purpose, first, we’ll get the reference of the target element in the JS file by querySelector() and then we’ll use
...
- querySelectorAll does the same function but it selects all the elements having the same CSS selector, in the sequence of the element's occurrence. It returns a Nodelist
...
- It is a function that is passed as an argument within another function.
- Example:
...
### Library
- It is a collection of packaged programs that are open to using for everyone.
- Example:
...
The results are:
  ...
  * console.log(demoP.innerText)<img src="images/console-innerText.png">
  * console.log(demoP.textContent)<img src="images/console-textContent.png">
  * console.log(demoP.innerHTML)<img src="images/console-innerHTML.png">
...
- Alternative to create an element
  ...
  - document.createTextNode()
    ...
    - When compared to innerHTML, it appends the dynamic text to existing text content that was initialized in the HTML rather than replacing it with the dynamic text.
  - insertBefore or appendChild
    - insertBefore
      - This approach adds the required new node as a child node before the parent node's existing child node.
    - Appendchild
...
- A centralized network architecture that classifies computers into two sections:
  - Client
    - Clients requests for or submit the data through the network with parameters included.
  - Server 
    - Servers are the response providers. It is a program that listens to the client’s requests and responds to them. It's the place where we do heavy processing and store the data. Even Servers can act as Clients for database servers.
...
- Many applications have useful functions. API helps us to communicate with those applications to program the application.
- Example:<br />
...
- By default we receive the response in the string format, we need to parse it into JSON.<br/>
<img src="images/api.PNG" width="60%">

- The `fetch` calls the API and gets the response and first `then` only converts the response to JSON format and second `then` is used to console the responses.
...
- URLs are designed to accept only certain characters in the standard 128-character ASCII character set. Reserved or unsafe ASCII characters that are not being used for their intended purpose, as well as characters not in this set, have to be encoded. URL encoding serves the purpose of replacing these non-conforming characters with a % symbol followed by two hexadecimal digits that represent the ASCII code of the character.<br/>
<img src="images/uri.png" width="60%">

### To handle errors : Use .catch()
- It takes a callback and the error is passed to as an argument to the callback.
- When the catch function is called when the error occurred is passed to this function and then the error is displayed using the callback function.
